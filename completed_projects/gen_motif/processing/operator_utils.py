...
def findInvertedRepeat(intergenic, size):
    ...
    for i in range(0,len(intergenic)-((2*size))):
        ...
        for j in range(i+size,maxAllowedSpacer):
            ...
            if compare == revCompSeq and i>6 and (j+size+5)<len(intergenic):   #added filter so operator isn't at very edge of interoperon region
...
def appendOperatorMetadata(homologListFile, knownOperator, perc_ident):
    ...
    with open(f'{homologListFile}', mode="rb") as f:
        ...
        operator = knownOperator
            ...
            #append operator and score for each intergenic region in homologList
        for i in homologList:
            i["operator"], i["score"] =  findOperatorInIntergenic(i["intergenic"], operator.upper())
            
            #get consensus operator and total number of sequences compared
        consensus_data = getConsensus(homologList, min_ident=perc_ident)[0]
        num_seqs = getConsensus(homologList, min_ident=perc_ident)[1]

        lowest_identity = str(homologList[-1]["identity"])

            #format into a pretty dictionary
        operator_data = {"input_seq": operator, "perc_ident":lowest_identity, "num_seqs":num_seqs, "motif": consensus_data}       
...
if __name__ == "__main__":
    ...
    with open(f'cache/homolog_metadata/{acc}', mode='rb') as f:
        ...
        for i in homologs:
            print(getBestInvertedRepeat(i["intergenic"]))
            
        #operators = getBestInvertedRepeat(homologs[0]["intergenic"])
        print(operators)
